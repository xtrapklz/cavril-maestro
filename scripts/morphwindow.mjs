...
const CX = 150, CY = 150, R = 116;          // viewBox 0 0 300 300

export class MaestroMorphWindow extends HandlebarsApplicationMixin(ApplicationV2) {
  static DEFAULT_OPTIONS = {
    id: "maestro-morph",
    classes: ["maestro-morph"],
    window: { title: "Maestro — Ambience Mixer", icon: "fa-solid fa-circle-nodes", resizable: false },
  ...
  async _prepareContext() {
    ...
    const tracks = info.tracks.map(t => {
      ...
      const lx = CX + (R + 18) * Math.cos(t.angle), ly = CY + (R + 18) * Math.sin(t.angle);
  ...
  _onRender() {
    ...
    if (!svg || !puck) return;
    const toSvg = ev => { const r = svg.getBoundingClientRect(); return { x: (ev.clientX - r.left) / r.width * 300, y: (ev.clientY - r.top) / r.height * 300 }; };
